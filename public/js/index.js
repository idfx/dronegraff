// Logo language controller for Parrot AR Drone 2
//
// Source Code Copyright (C) 2015 Juan Lara
//
    ...
    function send() {
        ...
        if (flg_run) {
            setTimeout(function () {
                try {
                    var logoScript = v;
                    // TODO: Connect to the server and send the commands
                    //logodrone.convertAndSend(logoScript);
                    socket.emit('send2drone', v);
                    error.appendChild(document.createTextNode("Script sent."));
                } catch (e) {
                    error.appendChild(document.createTextNode("There was an error sending your script."));
                }
            }, 1000000000000);
        }else{
            error.appendChild(document.createTextNode("You need to test your script first!"));
        }
        error.classList.add('shown');
    }
